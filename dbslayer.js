...
Server = this.Server = function(database, host, port, timeout){
...
Server.prototype.fetch = function(object, key){
  var connection = http.createClient(this.port, this.host),
      request = connection.request('GET', '/db?' + escape(JSON.stringify(object)), {'host': this.host}),
  ...
  request.finish(function(response){
    response.addListener('body', function(data){  
      ...
      if (object.MYSQL_ERROR !== undefined){
        ...
        promise.emitSuccess((key ? object[key] : object)[1]);
      }      
    });
  });
...
Server.prototype.query = function(query){
  // HACK for some reason db is not selected
  query = "USE " + this.database + ";" + query;
